# Improved Statistical Analysis for Publication
# Kohler Microbiome Study - Publication-Ready Methods
...
library(patchwork)
library(DESeq2)
library(ANCOMBC)  # install with: BiocManager::install("ANCOMBC")

# Load your phyloseq object (assuming it's already created)
# physeq <- [your phyloseq object]
# alpha_data_rarefied <- [your alpha diversity data]

# =============================================================================
# 1. POWER ANALYSIS AND SAMPLE SIZE REPORTING
# =============================================================================

# Create sample size summary table
sample_summary <- as.data.frame(sample_data(physeq_rarefied)) %>%
...
# =============================================================================
# 2. IMPROVED PERMANOVA WITH MATERNAL BLOCKING
# =============================================================================
...
# Create permutation design that respects maternal structure
library(permute)
perm_design <- how(nperm = 9999,
                   blocks = metadata$Mother)

# Run PERMANOVA with proper blocking
# This accounts for non-independence of siblings
set.seed(42)
permanova_blocked <- adonis2(
  bray_dist ~ SampleType + Age,
  data = metadata,
  permutations = perm_design,
  by = "margin"  # Type III sums of squares
)

print("PERMANOVA with Maternal Blocking:")
print(permanova_blocked)

# Also run standard PERMANOVA for comparison
...
print("Standard PERMANOVA (Mother as fixed effect):")
print(permanova_standard)

# =============================================================================
# 3. EFFECT SIZE SUMMARY TABLE
# =============================================================================
...
# =============================================================================
# 4. PERMDISP (CHECK ASSUMPTIONS)
# =============================================================================

# Test homogeneity of dispersion for each factor
...
print("SampleType:")
print(anova(dispersion_sampletype))
print("Age:")
print(anova(dispersion_age))
print("Mother:")
print(anova(dispersion_mother))

# =============================================================================
# 5. FIGURE 2: MATERNAL EFFECTS (3-PANEL)
# =============================================================================
...
# Recalculate ordination for subset
...
# =============================================================================
# 7. COMPOSITIONAL DATA ANALYSIS (ANCOM-BC)
# =============================================================================

# Run ANCOM-BC for differential abundance
# This is more appropriate for compositional data than DESeq2 alone

ancom_result <- ancombc(
  phyloseq = physeq_rarefied,
  formula = "SampleType",
  p_adj_method = "BH",
  group = "SampleType",
  struc_zero = TRUE,
...
ancom_res_df <- ancom_result$res %>%
  filter(diff_SampleTypeSterile == TRUE) %>%
  arrange(p_SampleTypeSterile)

print("ANCOM-BC Significant Taxa:")
print(head(ancom_res_df, 10))

# Save results
write.csv(ancom_res_df, "ANCOMBC_Results.csv", row.names = TRUE)
...
cat("  - Fig2_MaternalEffects.pdf\n")
cat("  - Fig3_EnvironmentalEffects.pdf\n")
cat("  - Table1_EffectSizes.csv\n")
cat("  - Table2_SummaryStats.csv\n")
cat("  - ANCOMBC_Results.csv\n")
cat("  - SessionInfo.txt\n")
